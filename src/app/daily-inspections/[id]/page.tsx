...
import { getCurrentUser, canApprove as canApproveInspection } from "@/lib/auth";
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        ...
        <Card className="p-4">
          ...
          <p className="text-2xl font-bold text-slate-900 mt-1">
            {i.totalItems > 0 ? Math.round((i.passedItems / i.totalItems) * 100) : 0}%
          </p>
        </Card>
        <Card className="p-4">
        ...
        <Card className="p-5 lg:col-span-2">
          ...
          {i.notes && (
            <div className="mt-4 pt-4 border-t border-slate-200">
              <p className="text-xs uppercase tracking-wider text-slate-500 font-semibold mb-1">Driver Notes</p>
            ...
            <div key={cat.category} className="border border-slate-200 rounded-lg overflow-hidden">
              <div className="bg-slate-50 px-4 py-2.5 border-b border-slate-200 flex items-center justify-between">
                ...
                <span className="text-xs text-slate-500">
                  {cat.items.filter((i) => i.result === "pass").length}/{cat.items.length} passed
                </span>
              </div>
                  ...
                  <div key={idx} className="px-4 py-2.5 flex items-start justify-between gap-3 text-sm">
                    <div className="flex-1 min-w-0">
                      ...
                      {(item.photos?.length || 0) > 0 && (
                        <div className="mt-1">
                          <PhotoGallery photos={(item.photos || []).map((p, i) => ({ id: String(i), dataUrl: p, takenAt: new Date().toISOString() }))} />
                        </div>
                      )}
                    </div>
