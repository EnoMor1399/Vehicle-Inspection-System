...
type LegacySection = {
  code: string;
  title: string;
  criteria: readonly LegacyCriterion[];
};
...
const RIGHT_SECTIONS: readonly LegacySection[] = [
  {
    code: "D",
    title: "Observation and Anticipation",
    criteria: [
  ...
  {
    code: "E",
    title: "Approaching junctions, turning and exiting",
    criteria: [
...
export default async function LegacyDriverAssessmentPrintPage({ params }: { params: Promise<{ assessmentId: string }> }) {
  ...
  const roadSignsRating = ratings.road_signs_markings;
  const roadSignsScore = typeof roadSignsRating === "number" ? `${roadSignsRating * 20}%` : "—";
  ...
  const regulationDisqualification = [
    "dangerous_speeding",
    "ignored_traffic_signal",
    "unsafe_overtaking",
    "failure_to_yield",
    "failure_to_stop",
    "dangerous_following",
    "reckless_aggressive",
    "vulnerable_user_violation",
  ].some((id) => critical.has(id));

  const additionalComments = assessment.remarks
    || assessment.qualitativeFeedback?.trainerComments
    || assessment.reviewComments
    || assessment.driverComments
    || "";
    ...
    <main className="legacy-print-page">
      ...
      <section className="legacy-a4-form" aria-label="Driver assessment A4 print form">
        ...
        <div className="meta-grid">
          <table className="meta-table" aria-label="Driver information">
            <tbody>
          ...
          <table className="meta-table" aria-label="Test and licence information">
            <tbody>
        ...
        <div className="bottom-grid">
          <div className="bottom-left">
            ...
            <div className="performance-box">
              ...
              <table>
                <tbody>
                  <tr><th>Theory</th><td>{formatPercent(assessment.theoryScore)}</td></tr>
                  <tr><th>Road Signs</th><td>{roadSignsScore}</td></tr>
                  <tr><th>Practical</th><td>{formatPercent(assessment.practicalScore ?? assessment.overallScore)}</td></tr>
                  <tr><th>Eye Test</th><td>—</td></tr>
                </tbody>
              </table>
            </div>

            <div className="comments-box">
              <div className="box-heading">Additional Comments</div>
              <div className="comments-text">{additionalComments}</div>
            </div>
          </div>

          <div className="bottom-right">
            <table className="disqualification-table">
              <thead><tr><th colSpan={2}>Mandatory Disqualification</th></tr></thead>
              <tbody>
            ...
            <table className="grading-table" aria-label="Assessment grading scale">
              <tbody>
            ...
            <div className="assessor-name-block">
              <div className="signature-rule" />
              <div>Name of Assessor</div>
              <strong>{assessor?.name || "—"}</strong>
              {reviewer?.name ? <small>Reviewed by: {reviewer.name}</small> : null}
            </div>
          </div>
        </div>
        ...
        <div className="signature-footer">
          <div><strong>Assessor Sign:</strong><span /></div>
          <div><strong>Mgr Sign:</strong><span /></div>
        </div>
        <div className="system-footnote">VIMS assessment record {session.referenceNumber} · Stored digital assessment remains the authoritative detailed criterion record.</div>
      </section>
    </main>
  );
}
...
function MetaRow({ label, value }: { label: string; value: string }) {
  return <tr><th>{label}</th><td colSpan={3}>{value}</td></tr>;
}
...
function CriteriaTable({
  ...
  return (
    <table className="criteria-table">
      {showHeader ? <thead><tr><th className="no-col">No.</th><th>Criteria observed</th><th className="score-col">Score</th><th className="award-col">Award</th></tr></thead> : null}
      <tbody>
        {sections.map((section) => (
          <SectionRows key={section.code} section={section} ratings={ratings} />
        ))}
        {showTotal ? (
          <tr className="total-row"><th colSpan={2}>TOTAL %</th><th>{LEGACY_MAX_SCORE}</th><th>{displayAward(totalAward)}</th></tr>
        ) : null}
      </tbody>
    </table>
  );
...
function SectionRows({ section, ratings }: { section: LegacySection; ratings: Record<string, number> }) {
  return (
    <>
      <tr className="section-row"><th colSpan={4}>{section.code} - {section.title}</th></tr>
      {section.criteria.map((item) => (
        <tr key={item.no} className="criterion-row">
          <td className="criterion-no">{item.no}</td>
          <td>{item.label}</td>
          <td className="numeric-cell">{item.weight}</td>
          <td className="numeric-cell award-cell">{displayAward(awardFor(item.weight, ratings[item.ratingId]))}</td>
        </tr>
      ))}
    </>
  );
...
function DisqualificationRow({ label, active }: { label: string; active: boolean }) {
  return <tr><th>{label}</th><td>{active ? "X" : ""}</td></tr>;
}
