...
function MenuButton({
  ...
  return (
    <details ref={detailsRef} onKeyDown={handleKeyDown} className="group relative shrink-0">
      <summary
      ...
      <div className="absolute left-0 z-40 mt-2 w-64 overflow-hidden rounded-xl border border-[var(--vims-line)] bg-[var(--vims-panel-solid)] p-1.5 shadow-xl">
        {items.map(({ href, label: itemLabel, icon: Icon }) => {
          ...
          return (
            <Link
              ...
              className={`flex items-center gap-2.5 rounded-lg px-3 py-2.5 text-sm font-semibold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--brand-color)] focus-visible:ring-inset ${
                itemActive
                  ? "bg-[var(--vims-panel-soft)] text-[var(--brand-color)]"
                  : "text-[var(--vims-ink-soft)] hover:bg-[var(--vims-panel-soft)] hover:text-[var(--vims-ink)]"
              }`}
            >
...
export default function DriverTrainingNav() {
  ...
  useEffect(() => {
    operationsRef.current?.removeAttribute("open");
    assessmentRef.current?.removeAttribute("open");
  ...
  return (
    <div className="border-b border-[var(--vims-line)] bg-[var(--vims-panel-solid)] px-4 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-[1600px] py-2.5">
        <nav aria-label="Driver Training & Assessment" className="flex min-w-0 items-start gap-1.5 overflow-x-auto pb-1 [scrollbar-width:thin] sm:overflow-visible sm:pb-0">
          <Link
            href="/driver-training"
            aria-current={pathname === "/driver-training" ? "page" : undefined}
          ...
          <MenuButton label="Operations" items={OPERATIONS} pathname={pathname} detailsRef={operationsRef} />
          <MenuButton label="Assessments" items={ASSESSMENT} pathname={pathname} detailsRef={assessmentRef} />

          <Link
            href="/driver-training/analytics"
            aria-current={isActivePath(pathname, "/driver-training/analytics") ? "page" : undefined}
            className={`inline-flex min-h-10 shrink-0 items-center gap-2 rounded-xl px-3.5 py-2 text-sm font-semibold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--brand-color)] focus-visible:ring-offset-2 ${
              isActivePath(pathname, "/driver-training/analytics")
                ? "bg-[var(--brand-color)] text-white shadow-sm"
                : "text-[var(--vims-ink-soft)] hover:bg-[var(--vims-panel-soft)] hover:text-[var(--vims-ink)]"
            }`}
          >
          ...
          <details ref={moreRef} onKeyDown={handleMoreKeyDown} className="group relative shrink-0">
            ...
            <div className="absolute right-0 z-40 mt-2 w-[min(92vw,34rem)] rounded-xl border border-[var(--vims-line)] bg-[var(--vims-panel-solid)] p-2 shadow-xl">
              <div className="grid gap-2 sm:grid-cols-2">
                {MORE_GROUPS.map((group) => (
                  <div key={group.label}>
                    <p className="px-2 pb-1 pt-1 text-[10px] font-bold uppercase tracking-[0.14em] text-[var(--vims-ink-muted)]">{group.label}</p>
                    {group.items.map(({ href, label, icon: Icon }) => {
                      ...
                      return (
                        <Link
                          ...
                          className={`flex items-center gap-2.5 rounded-lg px-2.5 py-2 text-sm font-semibold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--brand-color)] focus-visible:ring-inset ${
                            active
                              ? "bg-[var(--vims-panel-soft)] text-[var(--brand-color)]"
                              : "text-[var(--vims-ink-soft)] hover:bg-[var(--vims-panel-soft)] hover:text-[var(--vims-ink)]"
                          }`}
