  ...
  return (
    <div
      className={classNames(
        "rounded-2xl border border-slate-200/80 bg-white shadow-[0_1px_2px_rgba(15,23,42,0.025),0_12px_32px_rgba(15,23,42,0.045)] dark:border-slate-800 dark:bg-slate-900 dark:shadow-[0_1px_2px_rgba(0,0,0,0.18),0_16px_40px_rgba(0,0,0,0.22)]",
        className
      )}
    >
      {children}
  ...
  return (
    <div className="mb-5 flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
      <div className="min-w-0 flex-1">
        <h1 className="break-words text-2xl font-semibold tracking-[-0.025em] text-slate-950 dark:text-slate-50 sm:text-[28px]">{title}</h1>
        {description && (
          <p className="mt-1.5 max-w-3xl text-sm leading-5 text-slate-500 dark:text-slate-400">{description}</p>
        )}
      </div>
...
export function Button({
  ...
  const variants: Record<string, string> = {
    primary: "bg-slate-950 text-white shadow-sm hover:bg-slate-800 dark:bg-slate-100 dark:text-slate-950 dark:hover:bg-white",
    secondary: "border border-slate-300 bg-white text-slate-800 shadow-sm hover:border-slate-400 hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100 dark:hover:border-slate-600 dark:hover:bg-slate-800",
  ...
  return (
    <button
      type={type}
      className={classNames(
        "inline-flex items-center justify-center gap-2 rounded-xl font-semibold transition-[background-color,border-color,color,box-shadow,transform] duration-150 disabled:cursor-not-allowed disabled:opacity-50 active:translate-y-px focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-400 focus-visible:ring-offset-2 dark:focus-visible:ring-slate-500 dark:focus-visible:ring-offset-slate-950",
        variants[variant],
        sizes[size],
        className
      )}
...
export function StatCard({
  ...
  return (
    <Card className="group relative overflow-hidden p-4 sm:p-5">
      <span className="absolute inset-x-0 top-0 h-[2px] bg-slate-200 transition-colors group-hover:bg-[var(--brand-color)] dark:bg-slate-800" />
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          <p className="truncate text-xs font-medium uppercase tracking-[0.08em] text-slate-500 dark:text-slate-400">{label}</p>
          <p className="mt-2 break-words text-2xl font-semibold tracking-[-0.03em] text-slate-950 dark:text-slate-50 sm:text-[28px]">{value}</p>
...
export function EmptyState({ title, description, icon }: { title: string; description?: string; icon?: ReactNode }) {
  return (
    <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50/60 px-4 py-12 text-center dark:border-slate-700 dark:bg-slate-900/50 sm:py-14">
      {icon && <div className="mx-auto mb-4 grid h-11 w-11 place-items-center rounded-xl bg-white text-slate-400 shadow-sm ring-1 ring-slate-200 dark:bg-slate-800 dark:text-slate-500 dark:ring-slate-700">{icon}</div>}
      <p className="text-base font-semibold text-slate-900 dark:text-slate-100">{title}</p>
      {description && <p className="mx-auto mt-1.5 max-w-md text-sm leading-6 text-slate-500 dark:text-slate-400">{description}</p>}
    </div>
  );
}
...
export function Field({
  ...
  return (
    <label className="block">
      ...
      {children}
      {hint && <span className="mt-1.5 block text-xs leading-5 text-slate-500 dark:text-slate-400">{hint}</span>}
    </label>
  );
}

const controlClass =
  "block w-full rounded-xl border border-slate-300 bg-white px-3.5 py-2.5 text-base text-slate-900 shadow-[0_1px_2px_rgba(15,23,42,0.03)] placeholder:text-slate-400 transition-[border-color,box-shadow,background-color,color] hover:border-slate-400 focus:border-[var(--brand-color)] focus:outline-none focus:ring-4 focus:ring-slate-100 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100 dark:placeholder:text-slate-500 dark:hover:border-slate-600 dark:focus:ring-slate-800 sm:text-sm";
